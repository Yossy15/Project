...
class _AllPageState extends State<AllPage> {
  ...
  Future<void> handleImageUpload(String orderId) async {
    ...
    try {
      ...
      if (pickedFiles.isNotEmpty) {
        ...
        for (var image in images) {
          ...
          var multipartFile = http.MultipartFile(
            'images', stream, length, filename: image.path.split('/').last,
          );
        ...
        if (response.statusCode == 200) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('อัปโหลดรูปภาพสำเร็จ'), backgroundColor: Colors.green),
          );
          fetchAvailableOrders();
        } else {
      ...
      setState(() => isLoading = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('เกิดข้อผิดพลาดในการอัปโหลดรูปภาพ: $e'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Widget _buildOrderCard(dynamic order) {
    ...
    return Card(
      ...
      child: Column(
        children: [
          ListTile(
            leading: _buildOrderImage(order),
            title: Text(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                _buildStatusBadge(statusInfo['text'], statusInfo['color']),
              ],
            ),
            trailing: Row(
                ...
                ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => LunchPage(order: order),
                      ),
                    );
                  },
          ...
          if (order['imageUrls'].length > 1)
            _buildOrderImages(order['imageUrls']),
        ],
      ),
    );
  }

  Widget _buildOrderImage(dynamic order) {
    return Container(
      width: 60,
      height: 60,
  ...
  Widget _buildOrderImages(List<dynamic> imageUrls) {
    return Column(
      children: [
        const Divider(),
        SizedBox(
