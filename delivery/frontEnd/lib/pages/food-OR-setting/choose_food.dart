...
class _FoodOrderComponentState extends State<FoodOrderComponent> {
      ...
      if (storedUserId != null) {
        ...
        if (response.statusCode == 200) {
          ...
          if (responseData['status'] == true) {
            ...
            setState(() => message = 'ไม่สามารถดึงข้อมูลผู้ใช้ได้');
          }
        } else {
          setState(() => message = 'เกิดข้อผิดพลาดในการดึงข้อมูลผู้ใช้');
        }
      } else {
        setState(() => message = 'ไม่พบข้อมูลผู้ใช้ กรุณาเข้าสู่ระบบใหม่');
      }
    } catch (e) {
      setState(() => message = 'เกิดข้อผิดพลาด: $e');
    }
  }

  Future<void> createOrder() async {
    if (userId == null || userData == null) {
      setState(() => message = 'กรุณาเข้าสู่ระบบก่อนสั่งอาหาร');
      return;
    }
    ...
    if (userGpsLocation == null ||
      ...
      setState(() => message = 'ไม่พบข้อมูลตำแหน่ง GPS กรุณาอัปเดตตำแหน่งของคุณก่อนสั่งอาหาร');
      return;
    }
    setState(() {
      isLoading = true;
    ...
    try {
      ...
      if (response.statusCode == 201) {
        setState(() => message = 'คำสั่งซื้อสร้างสำเร็จ!');
        Navigator.push(
          context,
          MaterialPageRoute(
        ...
        setState(() => message = 'ไม่สามารถสร้างคำสั่งซื้อได้: ${response.body}');
      }
    } catch (e) {
      setState(() => message = 'เกิดข้อผิดพลาด: $e');
    } finally {
  ...
  Widget _buildFoodImage() => Container(
        ...
        decoration: BoxDecoration(
          ...
          image: DecorationImage(
            image: NetworkImage(widget.selectedFood.imageUrl),
            fit: BoxFit.cover,
          ),
        ),
      );
  ...
  Widget _buildQuantitySelector() => Row(
        ...
        children: [
          Text("Portion", style: GoogleFonts.lato(fontWeight: FontWeight.bold)),
          Row(
            children: [
              IconButton(
  ...
  Widget _buildOrderButton() => isLoading
